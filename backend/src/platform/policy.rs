...
// The bidirectional bridges to the framework's `AccessAction`/
// `PolicyAccess` that used to live here (backend framework replacement
// phase 7, slice 3) are deleted as of slice 8: confirmed dead by grep
// before removal -- `admin_ui.rs`'s `AdminPolicyExplainProvider`/
// `AdminAuditTimelineProvider` impls (the only stated reason for these
// bridges) both now use this crate's own self-owned `AccessAction`/
// `UserAuth` and the self-owned `AdminPolicyDecision: From<PolicyAccess>`
// (`platform::admin_runtime`) directly -- the framework-fixed admin trait
// entanglement that originally required these bridges was resolved by
// slice 6.3 without this file's comment being updated to say so. Nothing
// anywhere in `backend/src` constructed the framework-typed side any
// more.
