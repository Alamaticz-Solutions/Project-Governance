...
///
/// **Intentional product-owned divergence from the reference template**
/// (2026-09-07, manual DB bring-up gap #1): the framework's Postgres
/// `create_tables_sql/_mod.j2` never emitted a `CREATE SCHEMA`, so
/// `psql -f tables.pg.sql` against a fresh database failed immediately with
/// `schema "governance" does not exist` -- the old `scripts/appfw product
/// migrate` step created the schema out of band, and that step has no
/// replacement in `product_cli` (which does `generate` / `boundary-check` /
/// `validate` / `feature-check` only). The MSSQL and Snowflake table
/// templates both already create their schema inline
/// (`tables.mssql.sql` / `tables.snowflake.sql`); this brings the Postgres
/// dialect into line with them so the generated file is self-sufficient.
/// `CREATE SCHEMA IF NOT EXISTS` is idempotent, so re-running against an
/// already-provisioned database is a no-op. Same divergence rationale as
/// `pg_seed_literal_for_column` (commit ccaeee4).
pub fn render_create_tables_sql(plan: &DdlPlan) -> String {
    ...
    // Every distinct schema any generated object lands in, first-seen order.
    let mut schemas: Vec<&str> = Vec::new();
    let candidate_schemas = plan
        .sequences
        .iter()
        .map(|s| s.schema_name.as_str())
        .chain(plan.tables.iter().map(|t| t.schema_name.as_str()))
        .chain(plan.audit_tables.iter().map(|a| a.schema_name.as_str()))
        .chain(plan.junction_tables.iter().map(|j| j.schema_name.as_str()));
    for schema in candidate_schemas {
        if !schema.is_empty() && !schemas.contains(&schema) {
            schemas.push(schema);
        }
    }
    for schema in &schemas {
        out.push_str(&format!("    CREATE SCHEMA IF NOT EXISTS {schema};\n"));
    }
    if !schemas.is_empty() {
        out.push('\n');
    }
