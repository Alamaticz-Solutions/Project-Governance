# Governed Microsoft Graph write stack

The G1 controls every write this product makes to Microsoft Graph. Reads go
through the named-operation registry (spec 003); writes additionally pass the
full 8-component G1 stack before any request leaves the process.

**Status:** built. `scheduleViaGraph` and `cancelViaGraph` (online meetings) are
live-certified against a real tenant — meeting created and cancelled, both
outcomes recorded in the `graph_write_attempts` ledger and `audit_events` with
real Graph resource ids. Subscription management and SharePoint upload remain
`write_gated` pending a public HTTPS callback (spec 003 D2) and the document-
storage decision (spec 004 D1); `cancel_calendar_event` has no live pass
(scheduling via `/onlineMeetings` never creates an `/events` resource, so the
`CancelOnlineMeeting` / `DELETE /onlineMeetings/{id}` path is the certified one).

The sections below are the design the stack was built from.
...
| open-decisions P5 | The authoritative gate matrix isn't in the repo | G1.7 write policy roles remain provisional until it is |
| open-decisions Q5 | Which entities carry `audited` vs rely on `AuditEvent` | G1.8 writes to `AuditEvent` unconditionally |
